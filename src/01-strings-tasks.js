...
function extractNameFromTemplate(value) {
  const firstName = value.slice('Hello, '.length, value.lastIndexOf(' '));
  const lastName = value.slice(value.lastIndexOf(' ') + 1, -1);
...
function unbracketTag(str) {
  ...
  return str.slice(leftBracket + 1, rightBracket);
}
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((letter) => {
    const indexInNormalAlphabet = normalAlphabet.indexOf(letter);
    if (indexInNormalAlphabet === -1) {
      encodedStr.push(letter);
    } else {
      encodedStr.push(shiftedAlphabet[indexInNormalAlphabet]);
    }
  });
